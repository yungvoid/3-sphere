...
def construct_3_ball():
    ...
    if B_surface:
        ...
        small_surface_arcs = [generate_great_arc(num_points_on_small_arcs, d, p, np.arccos(np.dot(d, p))) for p in E for d in d_points[j:j+2]]
    ...
    if B_interiror:
        ...
        norm = np.linalg.norm(interior_arcs[np.random.randint(0, len(interior_arcs))][np.random.randint(0, num_points_on_interior_arcs)])
